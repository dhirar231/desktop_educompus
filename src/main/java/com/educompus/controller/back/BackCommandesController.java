...
public class BackCommandesController {
    ...
    private void ouvrirDetail(Commande cmd) {
        ...
        try {
            ...
            // ── Bouton SMS ──
            if (liv != null && liv.getPhoneNumber() != null && !liv.getPhoneNumber().isBlank()) {
                Region sepSms = new Region(); sepSms.setPrefHeight(1);
                sepSms.setStyle("-fx-background-color: -edu-border;");

                Button btnSms = new Button("📱  Envoyer SMS au client");
                btnSms.setStyle(
                    "-fx-background-color: #27ae60; -fx-text-fill: white;" +
                    "-fx-background-radius: 10px; -fx-font-weight: 700;" +
                    "-fx-padding: 9 14 9 14; -fx-cursor: hand;");
                btnSms.setMaxWidth(Double.MAX_VALUE);

                Label smsFeedback = new Label(); smsFeedback.setVisible(false);
                smsFeedback.setStyle("-fx-font-size: 11px;");

                final Livraison livFinal = liv;
                final Commande  cmdFinal = cmd;
                btnSms.setOnAction(ev -> {
                    btnSms.setDisable(true);
                    btnSms.setText("Envoi en cours…");
                    new Thread(() -> {
                        try {
                            TwilioSmsService sms = new TwilioSmsService();
                            String msg = construireMessageSms(cmdFinal, livFinal, lignes);
                            sms.envoyer(livFinal.getPhoneNumber(), msg);
                            javafx.application.Platform.runLater(() -> {
                                btnSms.setText("📱  Envoyer SMS au client");
                                btnSms.setDisable(false);
                                smsFeedback.setText("✔ SMS envoyé au " + livFinal.getPhoneNumber());
                                smsFeedback.setStyle("-fx-font-size: 11px; -fx-text-fill: #27ae60; -fx-font-weight: 700;");
                                smsFeedback.setVisible(true);
                            });
                        } catch (Exception ex) {
                            ex.printStackTrace(); // voir l'erreur complète dans la console
                            javafx.application.Platform.runLater(() -> {
                                btnSms.setText("📱  Envoyer SMS au client");
                                btnSms.setDisable(false);
                                smsFeedback.setText("Erreur SMS : " + ex.getMessage());
                ...
                secLiv.getChildren().addAll(sepSms, btnSms, smsFeedback);
            }
    ...
    private void addRow(VBox parent, String label, String val) {        HBox row = new HBox(8); row.setAlignment(Pos.CENTER_LEFT);
