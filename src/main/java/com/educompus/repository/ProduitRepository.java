...
public class ProduitRepository {
    ...
    public void insert(Produit p) throws SQLException {
        ...
        try {
            ps.setString(1, p.getNom()); ps.setString(2, p.getDescription());
            ps.setDouble(3, p.getPrix()); ps.setString(4, p.getType());
            ps.setString(5, p.getCategorie()); ps.setString(6, p.getImage());
            ps.setInt(7, p.getUserId()); ps.setInt(8, p.getStock());
            ps.executeUpdate();
    ...
    public boolean existsByNomTypeCategorie(String nom, String type, String categorie, int excludeId) throws SQLException {
        ...
        PreparedStatement ps = conn.prepareStatement(sql);
        try {
            ps.setString(1, nom.trim()); ps.setString(2, type.trim());
            ps.setString(3, categorie.trim()); ps.setInt(4, excludeId);
            ResultSet rs = ps.executeQuery();
    ...
    public void decrementeStock(int produitId, int quantite) throws SQLException {
        Connection conn = EducompusDB.getConnection();
        try { decrementeStock(conn, produitId, quantite); }
        finally { conn.close(); }
    }

    /** Version transactionnelle */
    public void decrementeStock(Connection conn, int produitId, int quantite) throws SQLException {
        String sql = "UPDATE produit SET stock = stock - ? WHERE id=? AND stock >= ?";
        PreparedStatement ps = conn.prepareStatement(sql);
        try {
            ps.setInt(1, quantite); ps.setInt(2, produitId); ps.setInt(3, quantite);
